...
import TestFactory from "./typedContract/constructors/test";
import Test from "./typedContract/contracts/test";
...
describe("test test", () => {
  ...
  before(async function setup(): Promise<void> {
    ...
    testFactory = new TestFactory(api, deployer);

    contract = new Test(
      (await testFactory.new(initialState)).address,
      deployer,
      api
    );
  });
