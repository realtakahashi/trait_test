...
import CommonsFactory from "./typedContract/constructors/commons";
import Commons from "./typedContract/contracts/commons";
...
describe("commons test", () => {
  let commonsFactory: CommonsFactory;
  let api: ApiPromise;
  let deployer: KeyringPair;
  
  let contract: Commons;
  ...
  before(async function setup(): Promise<void> {
    ...
    commonsFactory = new CommonsFactory(api, deployer);

    contract = new Commons(
      (await commonsFactory.new(initialState)).address,
      deployer,
      api
    );
  });
